...
![Hog API Gateway Banner](https://img2.leileihog.top/vLogo/hog-gateway-pic.png)
...
- **💬 拦截提示回显命中词** —— 敏感词审查拦截时的提示改为「检测到敏感词[命中词]，请求已停止。请切换对话。」，直接把命中的敏感词告诉用户，便于其自行修改提示词。
...
- **🖼️ 创意工坊（Image Studio）** —— 新增 `/image-studio` 页面，用户使用自己创建的 API 密钥直接调用中继生图，扣减密钥额度与钱包余额（密钥的分组、模型限制与 IP 白名单同样生效），每次固定生成 1 张。按模型自动分派中继端点：Gemini 图片系列（`gemini-2.5-flash-image`、`nano-banana` 等）走 chat 兼容通道，其余模型走 images 兼容通道；两条路由在中继内被重写回标准路径，复用既有转发与计费链路。
- **🎛️ 按模型投放参数** —— 前端维护各模型族能力表，把尺寸 / 比例 / 分辨率 / 质量 / 格式投放到上游真正接受的字段：OpenAI 系（`gpt-image-*`、`dall-e-*`）用像素 `size`，imagen 用比例并以 `quality` 表达图片尺寸，MiniMax 把比例折算为等比像素，通义万相 / `z-image` / `wan` 系与 `agnes-image-*` 用分辨率档位；模型不支持的参数不会发出，避免被上游拒绝。模型选择器下方新增说明，提示当前为通用配置、实际以模型接受的参数为准。
- **🗂️ 本地图库与大图预览** —— 生成结果保存在浏览器本地（IndexedDB），保留 3 天、最多 60 条，提供画廊视图、下载与大图预览；预览弹窗中提示词显示在图片下方并可一键复制，弹窗内容超高时可滚动查看。
- **📊 每日生图配额与用量** —— 系统设置 → 绘图设置新增每日份数（生成图片）上限（0 表示不限，最大 100000），按用户每个业务日计数：请求前预留额度，失败（HTTP ≥ 400）自动释放不计入；创意工坊顶部实时显示今日已用 / 剩余次数。
- **🤖 `agnes-image-*` 模型支持** —— 新增该系列模型的识别与参数映射，尺寸档位（1K/2K/3K/4K）与宽高比（含 21:9）分别投放到 `size` 与 `ratio` 字段。
