...
test('the header override CLI template picker confirms before filling the wildcard rule', async () => {
  const user = userEvent.setup()
  render(<ConfigurationHarness currentRow={editingChannel} />)
  await screen.findByDisplayValue('Existing channel')
  await user.click(screen.getByRole('tab', { name: /Request & Response/ }))
  ...
  await user.click(screen.getByRole('combobox', { name: 'CLI Template' }))
  await user.click(
    await screen.findByRole('option', { name: 'Passthrough Template' })
  )
  const dialog = await screen.findByRole('alertdialog', {
    name: 'Enable request header passthrough?',
  })
